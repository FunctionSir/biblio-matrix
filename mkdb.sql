...
-- 用户借书量检查(1)
CREATE TRIGGER CHK_READERS_BORROWED_CNT_READERS
ON READERS
AFTER INSERT, UPDATE
AS
BEGIN
	DECLARE @READERS_CNT INTEGER;
	DECLARE @RECORDS_CNT INTEGER;
	SELECT @READERS_CNT=SUM(CNT) FROM READERS;
	SELECT @RECORDS_CNT=COUNT(*) FROM RECORDS;
	IF (@READERS_CNT!=@RECORDS_CNT) BEGIN
		ROLLBACK TRANSACTION;
		RAISERROR('conflict found between borrowed books count and records count',16,1);
	END;
END;
GO

-- 用户借书量检查(2)
CREATE TRIGGER CHK_READERS_BORROWED_CNT_RECORDS
ON RECORDS
...
-- 为书名, 书作者, 读者姓名, 以及管理员姓名创建索引
CREATE INDEX INDEX_BOOKS_NAME ON BOOKS("NAME")
CREATE INDEX INDEX_BOOKS_AUTHOR ON BOOKS("AUTHOR")
CREATE INDEX INDEX_READERS_NAME ON READERS("NAME")
CREATE INDEX INDEX_ADMINS_NAME ON ADMINS("NAME")
GO
